...
public interface PokemonCatalogRepository extends JpaRepository<Poketex, Integer> {
    Page<Poketex> findByNameContainingIgnoreCaseOrPromptContainingIgnoreCase(String name, String prompt, Pageable pageable);
    ...
    Page<Poketex> getRandomPokemons(Pageable pageable);

    @Query(value = "SELECT * FROM poketex WHERE username =:username ORDER BY RANDOM()", nativeQuery = true)
    Page<Poketex> getRandomPokemonByUsername(String username, Pageable pageable);

    @CrossOrigin(origins = "http://localhost:3000")
    Page<Poketex> findByUsername(String username, Pageable pageable);

    @Query(value = "SELECT * FROM poketex p WHERE p.prompt ~* :wordList OR p.name ~* :wordList OR p.username = ANY(string_to_array(:wordList, ','))", nativeQuery = true)
